...
class CustomNumbers {
  OpenCustomNumbers() {
    cy.get(selectors.customNumberBtn).click();
  }
  ...
  SelectPayment() {
    ...
    cy.get("body").then((main) => {
      if (
        ...
        cy.get(selectors.dateInput).clear({ force: true }).type("0226{enter}", { force: true });
        cy.get(selectors.cvvInput).clear({ force: true }).type("887", { force: true });
        cy.get(selectors.emailInput).clear({ force: true }).type("email@gmail.com", { force: true });
        cy.get(selectors.addressInput).clear({ force: true }).type("street 1", { force: true });
        cy.get(selectors.zipInput).clear({ force: true }).type("56437", { force: true });
        cy.get(selectors.cityInput).clear({ force: true }).type("Texas", { force: true });
        cy.get(selectors.stateInput).clear({ force: true }).type("Texas", { force: true });
      } else {
  ...
  DeleteTestNumber() {
    ...
    cy.get(selectors.snackBar).should("have.text", " Phone Number Successfully Deleted. ")
  }
}
